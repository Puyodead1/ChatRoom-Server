...
import optic_fusion1.server.commands.ResetPassword;
import optic_fusion1.server.commands.UUIDLookup;
...
public class SocketServer {
      ...
      while (!this.socketAcceptor.isInterrupted() && this.socket.isBound()) {
        try {
                ...
                try {
                  ...
                  if (packetLength > maxPacketSize) {
                    ...
                    try {
                      ...
                      new IOException(
                          "Could not skip bytes for too large packet " + clientConnection.getAddress().getHostAddress(),
                          e).printStackTrace();
                      break;
                    }
                    continue;
                  ...
                  new IOException(
                      "Could not receive packet for client " + clientConnection.getAddress().getHostAddress(), e)
                          .printStackTrace();
                  break;
                }
              }
          ...
          if (!(e instanceof EOFException)
              && (!(e instanceof SocketException) || !e.getMessage().equalsIgnoreCase("Socket closed"))) {
      ...
      public void run() {
        for (ClientConnection clientConnection : clients.keySet()) {
          if ((clientConnection.getEncryptionKey() != null && clientConnection.getDecryptionKey() != null)
              || !clientConnection.isUsingEncryption()) {
  ...
  private void onRawPacketReceive(final ClientConnection clientConnection, byte[] packet) {
    if (clientConnection.getEncryptionKey() == null && clientConnection.isUsingEncryption()) {
      try {
        ...
        new IOException(
            "Could not create encryption key from/for client " + clientConnection.getAddress().getHostAddress(), e)
                .printStackTrace();
        clientConnection.terminateConnection();
      }
      return;
    ...
    if (clientConnection.getDecryptionKey() != null && clientConnection.isUsingEncryption()) {
      try {
        ...
        new IOException("Could not decrypt packet data for client " + clientConnection.getAddress().getHostAddress(), e)
            .printStackTrace();
      }
    }
  ...
  public void broadcastRawPacket(final byte[] packet) {
    for (ClientConnection clientConnection : this.clients.keySet()) {
      try {
        ...
        new Exception("Could not broadcast raw packet to client " + clientConnection.getAddress().getHostAddress(), e)
            .printStackTrace();
      }
    }
  }
  ...
  public void broadcastPacket(final IPacket packet) {
    for (ClientConnection clientConnection : this.clients.keySet()) {
      try {
        ...
        new Exception("Could not broadcast packet to client " + clientConnection.getAddress().getHostAddress(), e)
            .printStackTrace();
      }
    }
  }
  ...
  public boolean createAccount(CommandSender sender, String userName, String password) {
    if (DATABASE.containsUser(userName)) {
      sender.sendPacket(new MessagePacket(OpCode.MESSAGE,
          new Message(null, "The username '" + userName + "' is already taken").serialize(),
          MessagePacket.MessageChatType.SYSTEM));
    ...
    if (!allowInsecurePasswords) {
      ...
      try {
        if (hibp.isPlainPasswordPwned(password)) {
          sender.sendPacket(new MessagePacket(OpCode.MESSAGE,
              new Message(null, "The password is insecure use something else").serialize(),
              MessagePacket.MessageChatType.SYSTEM));
          return false;
        }
    ...
    sender.sendPacket(new MessagePacket(OpCode.MESSAGE,
        new Message(null, "Registered the username " + userName).serialize(), MessagePacket.MessageChatType.SYSTEM));
  ...
  private void loadPropertiesFile() {
    try {
      ...
      allowInsecurePasswords = Boolean
          .parseBoolean(SERVER_PROPERTIES.getProperty("allow-insecure-properties", "false"));
  ...
  public void handleInput() {
    ...
    while ((line = scanner.nextLine()) != null) {
      ...
      if (line.startsWith("/")) {
        ...
        broadcastPacket(new MessagePacket(OpCode.MESSAGE, new Message(null, line).serialize(),
            MessagePacket.MessageChatType.SYSTEM));
      }
    }
  }
